...
class Maker {
    ...
    function trigger()
    {
        ...
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false); // localde çalışması için eklendi
...
function linkyap($table,$column,$baslik){

    $link   = clearurl($baslik);
    $sorgu  = "select count(*) as sayi from $table where $column='$link' ";
    $run    = mysql_query($sorgu);
    $sayi   = mysql_fetch_assoc($run);
    $sayi   = $sayi["sayi"];
    ...
    if($sayi>0){
        ...
        $uzunluk     = strlen($link."-"); 
        $sorgu       = "select right($column,LENGTH($column)-$uzunluk) as sayi from icerik where"; 
        $sorgu      .= " $column like '$link-%' and"; 
        $sorgu      .= " right($column,LENGTH($column)-$uzunluk) REGEXP '^-?[0-9]+$' order by sayi desc limit 1 ;";
        $run         = mysql_query($sorgu);
        $sayi        = mysql_fetch_assoc($run);
        $sayi        = $sayi["sayi"];

        
        if($sayi=="" or $sayi==null)
        {
            $link = $link."-2";
        }
        else
    ...
    function cek_bebegim($adres,$proxy=false)
    {

        //proxy tanýmlamalarý yapýyoruz
        $proxy = array(
            0=> array('183.207.228.9','82'), //china
            1=> array('122.96.59.102','80'), //china
            2=> array('111.13.12.216','80'), //Turkey
            3=> array('124.202.182.218','8118') //china 
        );
                ...
                if(@!curl_errno($curl))
                {
                    ...
                    @$data['info'] = '<span style="color:green;">Took ' . $info['total_time'] . ' seconds to send a request to ' . $info['url'].'</span><br>';
                }
                else
                {
                    @$data['info'] = '<span style="color:red;">Error:' . curl_error($curl).'</span><br>';
                }*/
